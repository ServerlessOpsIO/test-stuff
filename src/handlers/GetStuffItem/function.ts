...
import { StuffItemKeys, StuffItemData, getKeys } from '../../lib/StuffItem.js'
 ...
 *
 * @returns Promise<StuffItemData>
 */
export async function getItem(itemKeys: StuffItemKeys): Promise<StuffItemData> {
    ...
    if ( typeof output.Item == 'undefined' ) {
        throw new Error('StuffItem not found')
    }

    return unmarshall(output.Item) as StuffItemData
}
...
export async function handler (event: APIGatewayProxyEvent, _: Context): Promise<APIGatewayProxyResult> {
    ...
    let body: string
    try {
        const itemData: StuffItemData = await getItem(
            itemKeys,
        )
        statusCode = 200
